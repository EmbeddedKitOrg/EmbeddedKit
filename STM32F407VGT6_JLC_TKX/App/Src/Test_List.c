...
void Test_List(void)
{
    ...
    if (!test_list)
    {
        EK_rSerialPrintf(TestQueue, "链表创建测试 ❌ - 创建失败\r\n");
        test_failure_count++;
        return;
    }

    EK_rSerialPrintf(TestQueue, "链表创建测试 ✅ - 创建成功\r\n");
    test_success_count++;
    ...
    if (node1 && node2 && node3 && node4)
    {
        ...
        {
            EK_rSerialPrintf(
                TestQueue, "小规模节点插入测试 ✅ - 已插入4个节点，链表长度: %d\r\n", test_list->List_Count);
            test_success_count++;
        }
        else
        {
            EK_rSerialPrintf(TestQueue, "小规模节点插入测试 ❌ - 链表长度不正确: %d\r\n", test_list->List_Count);
            test_failure_count++;
        }

        // 测试小规模升序排序
        EK_rSerialPrintf(TestQueue, "小规模链表排序中(升序)...\r\n");
        EK_Result_t sort_result = EK_rListSort(test_list, false);

        if (sort_result == EK_OK)
        {
            EK_rSerialPrintf(TestQueue, "小规模升序排序测试 ✅ - 排序成功\r\n");
            test_success_count++;
            ...
            if (sort_correct)
            {
                EK_rSerialPrintf(TestQueue, "小规模升序排序验证 ✅ - 排序正确\r\n");
                test_success_count++;
            }
            else
            {
                EK_rSerialPrintf(TestQueue, "小规模升序排序验证 ❌ - 排序错误\r\n");
                test_failure_count++;
            }
        }
        else
        {
            EK_rSerialPrintf(TestQueue, "小规模升序排序测试 ❌ - 排序失败\r\n");
            test_failure_count++;
        }

        // 测试小规模降序排序
        EK_rSerialPrintf(TestQueue, "小规模链表排序中(降序)...\r\n");
        sort_result = EK_rListSort(test_list, true);

        if (sort_result == EK_OK)
        {
            EK_rSerialPrintf(TestQueue, "小规模降序排序测试 ✅ - 排序成功\r\n");
            test_success_count++;
            ...
            if (sort_correct)
            {
                EK_rSerialPrintf(TestQueue, "小规模降序排序验证 ✅ - 排序正确\r\n");
                test_success_count++;
            }
            else
            {
                EK_rSerialPrintf(TestQueue, "小规模降序排序验证 ❌ - 排序错误\r\n");
                test_failure_count++;
            }
        }
        else
        {
            EK_rSerialPrintf(TestQueue, "小规模降序排序测试 ❌ - 排序失败\r\n");
            test_failure_count++;
        }
        ...
        {
            EK_rSerialPrintf(TestQueue, "小规模节点删除测试 ✅ - 已删除node2，链表长度: %d\r\n", test_list->List_Count);
            test_success_count++;
        }
        else
        {
            EK_rSerialPrintf(TestQueue, "小规模节点删除测试 ❌ - 删除失败，链表长度: %d\r\n", test_list->List_Count);
            test_failure_count++;
        }
    }
    else
    {
        EK_rSerialPrintf(TestQueue, "小规模节点创建测试 ❌ - 创建失败\r\n");
        test_failure_count++;
    }
    ...
    {
        EK_rSerialPrintf(TestQueue, "大规模节点插入测试 ✅ - 已插入25个节点，链表长度: %d\r\n", test_list->List_Count);
        test_success_count++;
    }
    else
    {
        EK_rSerialPrintf(TestQueue, "大规模节点插入测试 ❌ - 插入失败，期望25个，实际: %d\r\n", large_insert_success);
        test_failure_count++;
    }

    // 排序前打印前几个节点的值
    EK_rSerialPrintf(TestQueue, "排序前前5个节点值: ");
    EK_Node_t *debug_node = test_list->List_Dummy->Node_Next;
    for (int i = 0; i < 5 && debug_node != test_list->List_Dummy; i++)
    {
        EK_rSerialPrintf(TestQueue, "%u ", debug_node->Node_Order);
        debug_node = debug_node->Node_Next;
    }
    EK_rSerialPrintf(TestQueue, "\r\n");

    // 测试大规模升序排序
    EK_rSerialPrintf(TestQueue, "大规模链表排序中(升序)...\r\n");
    EK_Result_t large_sort_result = EK_rListSort(test_list, false);

    if (large_sort_result == EK_OK)
    {
        EK_rSerialPrintf(TestQueue, "大规模升序排序测试 ✅ - 排序成功\r\n");

        // 排序后打印所有节点的值
        EK_rSerialPrintf(TestQueue, "排序后所有节点值: ");
        debug_node = test_list->List_Dummy->Node_Next;
        for (int i = 0; i < 25 && debug_node != test_list->List_Dummy; i++)
        {
            EK_rSerialPrintf(TestQueue, "%u ", debug_node->Node_Order);
            debug_node = debug_node->Node_Next;
            if ((i + 1) % 10 == 0) EK_rSerialPrintf(TestQueue, "\r\n");
        }
        EK_rSerialPrintf(TestQueue, "\r\n");
        ...
        {
            EK_rSerialPrintf(TestQueue, "大规模升序排序验证 ✅ - 排序正确，验证了%d个节点\r\n", index);
            test_success_count++;
        }
        else
        {
            EK_rSerialPrintf(TestQueue, "大规模升序排序验证 ❌ - 排序错误或节点数不匹配\r\n");
            test_failure_count++;
        }
    }
    else
    {
        EK_rSerialPrintf(TestQueue, "大规模升序排序测试 ❌ - 排序失败\r\n");
        test_failure_count++;
    }

    // 测试大规模降序排序
    EK_rSerialPrintf(TestQueue, "大规模链表排序中(降序)...\r\n");
    large_sort_result = EK_rListSort(test_list, true);

    if (large_sort_result == EK_OK)
    {
        EK_rSerialPrintf(TestQueue, "大规模降序排序测试 ✅ - 排序成功\r\n");
        test_success_count++;
        ...
        if (large_sort_correct && index == 25)
        {
            EK_rSerialPrintf(TestQueue, "大规模降序排序验证 ✅ - 排序正确，验证了%d个节点\r\n", index);
            test_success_count++;
        }
        else
        {
            EK_rSerialPrintf(TestQueue, "大规模降序排序验证 ❌ - 排序错误或节点数不匹配\r\n");
            test_failure_count++;
        }
    }
    else
    {
        EK_rSerialPrintf(TestQueue, "大规模降序排序测试 ❌ - 排序失败\r\n");
        test_failure_count++;
    }

    EK_rSerialPrintf(TestQueue, "链表测试完成\r\n\r\n");
}
