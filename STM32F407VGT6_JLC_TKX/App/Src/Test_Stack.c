...
void Test_Stack(void)
{
    ...
    if (!test_stack)
    {
        EK_rSerialPrintf(TestQueue, "动态栈创建测试 ❌ - 创建失败\r\n");
        test_failure_count++;
        return;
    }

    EK_rSerialPrintf(TestQueue, "动态栈创建测试 ✅ - 创建成功，容量: 80字节\r\n");
    test_success_count++;
    ...
    if (result == EK_OK)
    {
        EK_rSerialPrintf(TestQueue, "静态栈创建测试 ✅ - 创建成功，容量: %u\r\n", sizeof(stack_buffer));
        test_success_count++;
    }
    else
    {
        EK_rSerialPrintf(TestQueue, "静态栈创建测试 ❌ - 创建失败\r\n");
        test_failure_count++;
    }
    ...
    for (int i = 0; i < 3; i++)
    {
        ...
        if (result == EK_OK)
        {
            EK_rSerialPrintf(TestQueue, "入栈uint32_t: %u\r\n", data);
            push_success++;
        }
        else
        {
            EK_rSerialPrintf(TestQueue, "入栈失败，数据: %u\r\n", data);
        }
    }
    ...
    for (int i = 0; i < 2; i++)
    {
        ...
        if (result == EK_OK)
        {
            EK_rSerialPrintf(TestQueue, "入栈uint16_t: %u\r\n", data);
            push_success++;
        }
        else
        {
    ...
    if (result == EK_OK)
    {
        EK_rSerialPrintf(TestQueue, "入栈uint8_t: 0x%02X\r\n", byte_data);
        push_success++;
    }
    else
    {
        EK_rSerialPrintf(TestQueue, "入栈失败，数据: 0x%02X\r\n", byte_data);
    }

    if (push_success == push_total)
    {
        EK_rSerialPrintf(TestQueue, "栈入栈测试 ✅ - %d/%d 成功\r\n", push_success, push_total);
        test_success_count++;
    }
    else
    {
    ...
    EK_rSerialPrintf(TestQueue, "栈剩余空间: %u字节\r\n", EK_sStackGetRemain(test_stack));
    EK_rSerialPrintf(TestQueue,
                     "栈状态 - 空: %s, 满: %s\r\n",
    ...
    if (result == EK_OK)
    {
        EK_rSerialPrintf(TestQueue, "出栈uint8_t: 0x%02X\r\n", pop_byte);
        if (pop_byte == 0xFF) // 验证数据正确性
        {
            EK_rSerialPrintf(TestQueue, "uint8_t数据验证 ✅\r\n");
        }
        else
        {
            EK_rSerialPrintf(TestQueue, "uint8_t数据验证 ❌ - 期望0xFF，实际0x%02X\r\n", pop_byte);
        }
        pop_success++;
    }
    else
    {
        EK_rSerialPrintf(TestQueue, "出栈uint8_t失败\r\n");
    }
    ...
    for (int i = 1; i >= 0; i--)
    {
        ...
        if (result == EK_OK)
        {
            EK_rSerialPrintf(TestQueue, "出栈uint16_t: %u\r\n", pop_data);
            uint16_t expected = (i + 1) * 50;
            if (pop_data == expected)
            {
                EK_rSerialPrintf(TestQueue, "uint16_t数据验证 ✅\r\n");
            }
            else
            {
                EK_rSerialPrintf(TestQueue, "uint16_t数据验证 ❌ - 期望%u，实际%u\r\n", expected, pop_data);
            }
            pop_success++;
        }
        else
        {
            EK_rSerialPrintf(TestQueue, "出栈uint16_t失败\r\n");
        }
    }
    ...
    for (int i = 2; i >= 0; i--)
    {
        ...
        if (result == EK_OK)
        {
            EK_rSerialPrintf(TestQueue, "出栈uint32_t: %u\r\n", pop_data);
            uint32_t expected = (i + 1) * 100 + test_counter;
            if (pop_data == expected)
            {
                EK_rSerialPrintf(TestQueue, "uint32_t数据验证 ✅\r\n");
            }
            else
            {
                EK_rSerialPrintf(TestQueue, "uint32_t数据验证 ❌ - 期望%u，实际%u\r\n", expected, pop_data);
            }
            pop_success++;
        }
        else
        {
            EK_rSerialPrintf(TestQueue, "出栈uint32_t失败\r\n");
        }
    }

    if (pop_success == pop_total && pop_total > 0)
    {
        EK_rSerialPrintf(TestQueue, "栈出栈测试 ✅ - %d/%d 成功\r\n", pop_success, pop_total);
        test_success_count++;
    }
    else
    {
    ...
    EK_rSerialPrintf(TestQueue, "出栈后栈剩余空间: %u字节\r\n", EK_sStackGetRemain(test_stack));
    EK_rSerialPrintf(TestQueue,
                     "出栈后栈状态 - 空: %s, 满: %s\r\n",
                     EK_bStackIsEmpty(test_stack) ? "是" : "否",
                     EK_bStackIsFull(test_stack) ? "是" : "否");
    ...
    EK_rSerialPrintf(TestQueue, "填充测试 - 成功压入%d个uint32_t数据\r\n", fill_count);
    EK_rSerialPrintf(TestQueue, "栈剩余空间: %u字节\r\n", EK_sStackGetRemain(test_stack));
    ...
    {
        EK_rSerialPrintf(TestQueue, "栈满溢出保护测试 ✅ - 正确阻止溢出\r\n");
        test_success_count++;
    }
    else
    {
        EK_rSerialPrintf(TestQueue, "栈满溢出保护测试 ❌ - 未正确阻止溢出\r\n");
        test_failure_count++;
    }
    ...
    EK_rSerialPrintf(TestQueue, "清空栈 - 成功出栈%d个数据\r\n", empty_count);
    ...
    if (result == EK_EMPTY)
    {
        EK_rSerialPrintf(TestQueue, "空栈保护测试 ✅ - 正确阻止空栈出栈\r\n");
        test_success_count++;
    }
    else
    {
        EK_rSerialPrintf(TestQueue, "空栈保护测试 ❌ - 未正确阻止空栈出栈\r\n");
        test_failure_count++;
    }
    ...
    EK_rSerialPrintf(TestQueue, "静态栈压入测试 - 成功压入%d个数据\r\n", static_push_success);
    ...
    {
        EK_rSerialPrintf(TestQueue, "静态栈LIFO测试 ✅ - 数据验证正确\r\n");
        test_success_count++;
    }
    else
    {
        EK_rSerialPrintf(TestQueue, "静态栈LIFO测试 ❌ - 数据验证失败\r\n");
        test_failure_count++;
    }
    ...
    if (result == EK_OK)
    {
        EK_rSerialPrintf(TestQueue, "动态栈删除测试 ✅ - 删除成功\r\n");
        test_success_count++;
    }
    else
    {
        EK_rSerialPrintf(TestQueue, "动态栈删除测试 ❌ - 删除失败\r\n");
        test_failure_count++;
    }
    ...
    if (result == EK_OK)
    {
        EK_rSerialPrintf(TestQueue, "静态栈清理测试 ✅ - 清理成功\r\n");
        test_success_count++;
    }
    else
    {
        EK_rSerialPrintf(TestQueue, "静态栈清理测试 ❌ - 清理失败\r\n");
        test_failure_count++;
    }

    EK_rSerialPrintf(TestQueue, "栈测试完成\r\n\r\n");
}