...
void shellPrint(SHELL_TypeDef *shell, char *fmt, ...)
{
    ...
    va_start(vargs, fmt);
    lwvsnprintf(buffer, SHELL_PRINT_BUFFER - 1, fmt, vargs);
    va_end(vargs);
...
static void shellHistory(SHELL_TypeDef *shell, unsigned char dir)
{
    ...
    {
        if ((shell->length =
                 shellStringCopy(shell->buffer,
                                 shell->history[(shell->historyFlag + SHELL_HISTORY_MAX_NUMBER + shell->historyOffset) %
...
static void shellEnter(SHELL_TypeDef *shell)
{
    ...
    *(shell->buffer + shell->length++) = 0;
    ...
    {
        if ((quotes != 0 ||
             (*(shell->buffer + i) != ' ' && *(shell->buffer + i) != '\t' && *(shell->buffer + i) != ',')) &&
            *(shell->buffer + i) != 0)
        {
            if (*(shell->buffer + i) == '\"')
            {
...
static void shellTab(SHELL_TypeDef *shell)
{
    ...
    if (SHELL_GET_TICK())
    {
        if (matchNum == 1 && shell->status.tabFlag == 1 &&
