...
#        define SHELL_EXPORT_CMD(cmd, func, desc)                                    \
            ...
            const SHELL_CommandTypeDef shellCommand##cmd SECTION("shellCommand") = { \
                shellCmd##cmd, (int (*)())func, shellDesc##cmd, (void *)0}
#        define SHELL_EXPORT_CMD_EX(cmd, func, desc, help)                           \
            const char shellCmd##cmd[] = #cmd;                                       \
            const char shellDesc##cmd[] = #desc;                                     \
            const char shellHelp##cmd[] = #help;                                     \
            const SHELL_CommandTypeDef shellCommand##cmd SECTION("shellCommand") = { \
                shellCmd##cmd, (int (*)())func, shellDesc##cmd, shellHelp##cmd}
#    else /** SHELL_LONG_HELP == 1 */
#        define SHELL_EXPORT_CMD(cmd, func, desc)                                    \
            const char shellCmd##cmd[] = #cmd;                                       \
            const char shellDesc##cmd[] = #desc;                                     \
            const SHELL_CommandTypeDef shellCommand##cmd SECTION("shellCommand") = { \
                shellCmd##cmd, (int (*)())func, shellDesc##cmd}
#        define SHELL_EXPORT_CMD_EX(cmd, func, desc, help)                           \
            const char shellCmd##cmd[] = #cmd;                                       \
            const char shellDesc##cmd[] = #desc;                                     \
            const SHELL_CommandTypeDef shellCommand##cmd SECTION("shellCommand") = { \
...
#        define SHELL_EXPORT_VAR(var, variable, desc, type)                             \
            ...
            const SHELL_VaribaleTypeDef shellVariable##var SECTION("shellVariable") = { \
                shellVar##var, (void *)(variable), shellDesc##var, type}
#    else
#        define SHELL_EXPORT_VAR(var, variable, desc, type)
...
#        define SHELL_CMD_ITEM(cmd, func, desc)         \
            {                                           \
                #cmd, (int (*)())func, #desc, (void *)0 \
            }
#        define SHELL_CMD_ITEM_EX(cmd, func, desc, help) \
            {                                            \
                #cmd, (int (*)())func, #desc, #help      \
            }
#    else /** SHELL_LONG_HELP == 1 */
#        define SHELL_CMD_ITEM(cmd, func, desc) \
            {                                   \
