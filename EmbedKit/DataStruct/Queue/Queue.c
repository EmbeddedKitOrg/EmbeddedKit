 ...
 * @return DS_Result_t 入队成功返回DS_SUCCESS，失败返回对应错误码
...
{
    // 参数有效性检查
    if (queue == NULL || data == NULL || data_size == 0) return DS_ERROR_INVALID_PARAM;

    // 检查队列是否满了
    if (QueueIsFull(queue)) return DS_ERROR_FULL;

    // 检测剩余空间是否足够
    if (QueueGetRemain(queue) < data_size) return DS_ERROR_INSUFFICIENT_SPACE;
 ...
 * @return DS_Result_t 操作结果，成功返回DS_SUCCESS
 * @note 会检查队列中数据是否足够，数据会从队列中移除并复制到data_buffer中
 */
DS_Result_t QueueDequeue(Queue_t *queue, void *data_buffer, size_t data_size)
{
    // 参数有效性检查
    if (queue == NULL || data_buffer == NULL || data_size == 0) return DS_ERROR_INVALID_PARAM;

    // 检查队列是否为空
    if (QueueIsEmpty(queue)) return DS_ERROR_EMPTY;

    // 检查队列是否有所需求的数据数目
    if (QueueGetSize(queue) < data_size) return DS_ERROR_INSUFFICIENT_SPACE;
