...
A: `main()` 是 C 语言的标准入口，由 L1_MCU 层实现，负责硬件初始化（时钟、中断等）。`ek_main()` 是本项目自定义的应用层入口，由 `main()` 调用，专注于业务逻辑。这样分离后，业务代码与硬件初始化解耦。
...
**Q: L5 层可以直接调用 L2_Core/hal 吗？**

A: 不建议。L5 层应该通过 L4 组件访问硬件，而不是直接调用 L2。这样保持层次清晰，便于维护和测试。例如，需要控制 LED 时，应该使用 L4 的 `dev_led` 组件，而不是直接调用 `hal_gpio_write`。
